...
def normalize_hand_shoulder_dist(hand, dist):
    ...
    mask = hand[:, 0] != OUT_OF_FRAME_NUM
    hand[mask] -= wrist_copy
    hand[mask] /= dist
...
def normalize_face_shoulder_dist(face, dist):

    nose = face[NOSE_INDEX]
    nose_copy = nose.copy()
...
def normalize_shoulder_dist(pose_data_list, l_shoulder_index, r_shoulder_index):

    pose_array_list = []
    
    for frame_pose in pose_data_list:

        if (frame_pose['main_pose'][l_shoulder_index][0] == OUT_OF_FRAME_NUM or 
            frame_pose['main_pose'][r_shoulder_index][0] == OUT_OF_FRAME_NUM):
            
            frame_pose['l_hand'][:] = OUT_OF_FRAME_NUM
            frame_pose['r_hand'][:] = OUT_OF_FRAME_NUM
            frame_pose['main_pose'][:] = OUT_OF_FRAME_NUM
            frame_pose['face'][:] = OUT_OF_FRAME_NUM
            continue

        l_shoulder = frame_pose['main_pose'][l_shoulder_index]
        r_shoulder = frame_pose['main_pose'][r_shoulder_index]
        neck = (l_shoulder + r_shoulder) / 2
        dist = np.linalg.norm(l_shoulder - r_shoulder)

        l_wrist = normalize_hand_shoulder_dist(frame_pose['l_hand'], dist)
        r_wrist = normalize_hand_shoulder_dist(frame_pose['r_hand'], dist)
        nose = normalize_face_shoulder_dist(frame_pose['face'], dist)
        
        mask = frame_pose['main_pose'][:, 0] != OUT_OF_FRAME_NUM
        frame_pose['main_pose'][mask] -= neck
        frame_pose['main_pose'][mask] /= dist
        ...
        frame_pose_array = np.concatenate([
            frame_pose['l_hand'],
            frame_pose['r_hand'],
            frame_pose['main_pose'],
            frame_pose['face']
        ])
        
        pose_array_list.append(frame_pose_array)
    
    norm_pose_array = np.stack(pose_array_list)
    return norm_pose_array.reshape(norm_pose_array.shape[0], -1)
    ...
    for data_path in data_paths:
        ...
        with h5py.File(data_path, 'a') as f:
            for _, video_group in tqdm(f.items()):
                ...
                assert pose_data.shape[1] == NUM_LANDMARKS, f'Unexpected number of keypoints: {pose_data.shape[1]}'

                if pose_data.shape[0] == 0:
                    normalized_pose_data = np.empty((0, num_coords*(2*NUM_HAND_LANDMARKS + len(main_landmarks_list) + len(FACE_INDICES))), dtype=np.float32)
                    ...
                    selected_pose_data_list = select_pose_landmarks(pose_data, main_landmarks_list, frame_freq)
                    normalized_pose_data = normalize_shoulder_dist(selected_pose_data_list, l_shoulder_index, r_shoulder_index)

                if args.rewrite and 'processed_keypoints' in video_group:
                    del video_group['processed_keypoints']
                
                ds = video_group.create_dataset('processed_keypoints', data=normalized_pose_data, compression='gzip', compression_opts=6)
                ds.attrs['description'] = f'norm_shoulder_dist, hand_centered, face_centered, frame_freq={frame_freq}, use_z={use_z}, include_face={main_landmarks_config["include_face"]}, include_hands={main_landmarks_config["include_hands"]}, include_hips={main_landmarks_config["include_hips"]}'
